...
export default function DashboardView({ role }: DashboardProps) {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      if (!scriptUrl) {
        // MOCK DATA (Dados Fictícios) - Isso só aparece se a URL do Google Script não estiver configurada
        // Quando você publicar no Netlify com a variável VITE_GOOGLE_SCRIPT_URL configurada, 
        // esses dados sumirão e darão lugar aos dados reais da sua planilha.
        setGastos({
          total: 125000,
          porGrupo: {
            'Vertebrados': 45000,
          ...
          porMes: [
            { mes: '01/2024', Gasto: 15000 },
            { mes: '02/2024', Gasto: 25000 },
            { mes: '03/2024', Gasto: 40000 },
            { mes: '04/2024', Gasto: 45000 }
          ],
          recentes: [
            { data: '10/04/2024', descricao: 'Compra de armadilhas', valor: 1500, grupo: 'Artrópodes', comprovante: 'Sim' },
            { data: '08/04/2024', descricao: 'Diárias campo', valor: 760, grupo: 'Vertebrados', comprovante: 'Sim' },
            { data: '05/04/2024', descricao: 'Sequenciamento', valor: 5000, grupo: 'DNA Ambiental', comprovante: 'Não' },
            { data: '02/04/2024', descricao: 'Material botânico', valor: 800, grupo: 'Plantas', comprovante: 'Sim' },
            { data: '28/03/2024', descricao: 'Passagens', valor: 1200, grupo: 'Vertebrados', comprovante: 'Não' }
          ],
          metasStats: [
            { name: 'Concluídas', value: 12 },
            { name: 'Em Andamento', value: 10 },
            { name: 'Atrasadas', value: 6 }
          ],
          metasPorGrupo: {
            'Vertebrados': [
              { nome: 'Etapa 2 - Compra de Equipamentos', status: 'Em Andamento', progresso: 60 }
            ],
            'DNA Ambiental': [
              { nome: 'Etapa 3 - Coleta de Amostras', status: 'Atrasada', progresso: 30 }
            ]
          },
          gastosPorRubricaGrupo: {
            'Vertebrados': { 'Diárias': 15000, 'Material de Consumo': 30000 },
            'Artrópodes': { 'Material de Consumo': 30000 }
          },
          logisticaGrupo: {
            'Vertebrados': {
              emTransito: [{ descricao: 'Armadilhas', data: '10/04/2024' }],
              pendentesComprovante: [{ descricao: 'Passagens', valor: 1200, data: '28/03/2024' }],
              estoque: { 'Laboratório': 5, 'Sala de Projetos': 2 }
            }
          }
        });
        setLoading(false);
      ...
      if (result.success && result.data) {
        ...
        if (result.data.length === 0) {
          console.log("Planilha vazia, usando dados fictícios para demonstração.");
          setGastos({
            total: 125000,
            porGrupo: {
              'Vertebrados': 45000,
